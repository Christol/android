...
//                    break;
    ...
    ModbusID3ManagerListener mModbusID3ManagerListener;

    public void setModbusID3ManagerListener(ModbusID3ManagerListener modbusID3ManagerListener) {
        mModbusID3ManagerListener = modbusID3ManagerListener;
    }

    public interface ModbusID3ManagerListener {
        void resetArmPositionID3(int mOffset, String readValue, int writeValue, short[] mRegValues,boolean state);

        void middleWirteArmPositionID3(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state);

        void WirteArmPositionID3(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state);

        void ReadArmPositionID3(int mOffset, String value);
    }

    public ModbusID3Manager(ModbusTcpManager modbusManager) {
    ...
    public void resetReadP201DeviceMode() {//读取寄存器201 自动模式(P201=257)，
        ...
        resetAutoDeviceID3(ID3 + "", address, "1");//地址要填16进制
    }
    ...
    public void resetWirteP282DeviceReleaseBrake(boolean flag) {//P282 写入 16，即在伺服使能状态下，松开抱闸  十进制
        ...
        resetwriteAutoDeviceID3(ID3 + "", address, "16", false, flag);
    }
    ...
    public void resetWirteDeviceP290P291(String value, boolean flag) {//P290(高 16 位)与 P291(低 16 位)写入 32 位的指令位置
        ...
        Log.d(TAG, "ID3 resetWirteDeviceP290P291 value : " + value);
        ResetSend16(ID3 + "", address, value, false);
    }

    public void resetReadDeviceP290P291() {//读取 P290(高 16 位)
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P290);
        resetAutoDeviceID3(ID3 + "", address, "2");//地址要填16进制
    }

//    public void resetWirteDeviceP290Hight(String value, boolean flag) {//P290(高 16 位)与 P291(低 16 位)写入 32 位的指令位置
//        String address = ByteUtil.decimal2fitHex(ModbusConstans.P290);
//        resetwriteAutoDeviceID3(ID3 + "", address, value, false, flag);
//    }
//
...
//    }
    ...
    public void restWirteDeviceP282Data0() {//P282 写入 0，锁紧抱闸  十进制
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P282);
        resetwriteAutoDeviceID3(ID3 + "", address, "0", false, true);
    }
...
//    }
    ...
    public void middleWriteDeviceP282Data0() {//P282 写入 0，锁紧抱闸  十进制
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P282);
        middleWriteAutoDeviceID3(ID3 + "", address, "0", false, true);
    }
    ...
    public void readDeviceP201() {//读取寄存器201 自动模式(P201=257)，
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P201);
        readAutoDeviceID3(ID3 + "", address, "1");//地址要填16进制
    }
    ...
    public void writeDeviceP282(boolean flag) {//P282 写入 16，即在伺服使能状态下，松开抱闸  十进制
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P282);
        writeAutoDeviceID3(ID3 + "", address, "16", false, flag);
    }
    ...
    public void WirteDeviceP290P291(String value, boolean flag) {//P290(高 16 位)与 P291(低 16 位)写入 32 位的指令位置
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P290);
        Log.d(TAG, "ID3 WirteDeviceP290P291 value : " + value);
        Send16(ID3 + "", address, value, false);
    }

    public void ReadDeviceP290P291() {//读取 P290(高 16 位)
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P290);
        readAutoDeviceID3(ID3 + "", address, "2");//地址要填16进制
    }

//    public void writeDeviceP290Hight(String value, boolean flag) {//P290(高 16 位)与 P291(低 16 位)写入 32 位的指令位置
//        String address = ByteUtil.decimal2fitHex(ModbusConstans.P290);
//        writeAutoDeviceID3(ID3 + "", address, value, false, flag);
//    }
//
...
//    }
    ...
    public void writeDeviceP282Data0() {//P282 写入 0，锁紧抱闸  十进制
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P282);
        writeAutoDeviceID3(ID3 + "", address, "0", false, true);
    }
    ...
    public void readManualDeviceP201() {//读取寄存器201 自动模式(P201=257)，
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P201);
        readManualDeviceID3(ID3 + "", address, "1");//地址要填16进制
    }
    ...
    public void readManualDeviceP214P215() {//P214 反馈位置(高) 只读 反馈位置，P214 高 16 位，P215 低 16 位
        String address = ByteUtil.decimal2fitHex(ModbusConstans.P214);
        readManualDeviceID3(ID3 + "", address, "2");//地址要填16进制
    }
...
//    }
     ...
     *读保持寄存器
     */
    public void resetAutoDeviceID3(String salveId, String offset, String amount) {//读保持寄存器
...
//                            showOneToast("F03读取：\n" + ByteUtil.bytes2HexStr(data) + "\n");
                            if (mModbusID3ManagerListener != null) {
                                mModbusID3ManagerListener.resetArmPositionID3(mOffset, ByteUtil.bytes2HexStr(data), -1, null,true);
                            }
                        }
    ...
    public void resetwriteAutoDeviceID3(String salveId, String offset, String singleValue, boolean hexadecimal, final boolean flag) {//写单个寄存器
        int mSalveId = ModbusUtils.checkSlave(salveId);
        final int mOffset = ModbusUtils.checkOffset(offset);
...
//                                    showOneToast("F06写入成功\n");
                                    mModbusID3ManagerListener.resetArmPositionID3(mOffset, "-1", mRegValue,null, flag);
                                }
...
//                                    showOneToast("F16写入成功\n");
                                    mModbusID3ManagerListener.resetArmPositionID3(mOffset, "-1",-1, mRegValues, true);
                                }
     ...
     *写保持寄存器
     */
    private void middleReadAutoDeviceID3(String salveId, String offset, String amount) {
        int mSalveId = ModbusUtils.checkSlave(salveId);
        final int mOffset = ModbusUtils.checkOffset(offset);
        int mAmount = ModbusUtils.checkAmount(amount);
        if (mSalveId != -1 && mOffset != -1 && mAmount != -1) {
...
//                            showOneToast("F03读取：\n" + ByteUtil.bytes2HexStr(data) + "\n");
                            if (mModbusID3ManagerListener != null) {
                                mModbusID3ManagerListener.middleWirteArmPositionID3(mOffset, ByteUtil.bytes2HexStr(data), -1, null,true);
                            }
                        }
    ...
    private void middleWriteAutoDeviceID3(String salveId, String offset, String singleValue, boolean hexadecimal, final boolean flag) {
        int mSalveId = ModbusUtils.checkSlave(salveId);
        final int mOffset = ModbusUtils.checkOffset(offset);
        final int mRegValue = ModbusUtils.checkRegValue(singleValue, hexadecimal);
        if (mSalveId != -1 && mOffset != -1 && mRegValue != -1) {
            ...
            ModbusManager.get()
                    .writeSingleRegister(mSalveId, mOffset, mRegValue,
                            new ModbusCallback<WriteRegisterResponse>() {
                                ...
                                public void onSuccess(WriteRegisterResponse writeRegisterResponse) {
//                                    showOneToast("F06写入成功\n");
                                    mModbusID3ManagerListener.middleWirteArmPositionID3(mOffset, "-1",mRegValue,null, flag);
                                }
                    ...
                    .writeRegisters(mSalveId, mOffset, mRegValues,
                            new ModbusCallback<WriteRegistersResponse>() {
                                ...
                                public void onSuccess(WriteRegistersResponse writeRegistersResponse) {
                                    // 发送成功
//                                    showOneToast("F16写入成功\n");
                                    mModbusID3ManagerListener.middleWirteArmPositionID3(mOffset, "-1",-1, mRegValues, true);
                                }
     ...
     *写保持寄存器
     */
    public void readAutoDeviceID3(String salveId, String offset, String amount) {//读保持寄存器
        int mSalveId = ModbusUtils.checkSlave(salveId);
        final int mOffset = ModbusUtils.checkOffset(offset);
        int mAmount = ModbusUtils.checkAmount(amount);
        if (mSalveId != -1 && mOffset != -1 && mAmount != -1) {
...
//                            showOneToast("F03读取：\n" + ByteUtil.bytes2HexStr(data) + "\n");
                            if (mModbusID3ManagerListener != null) {
                                mModbusID3ManagerListener.WirteArmPositionID3(mOffset, ByteUtil.bytes2HexStr(data), -1, null,true);
                            }
                        }
     ...
     */
    public void writeAutoDeviceID3(String salveId, String offset, String singleValue, boolean hexadecimal, final boolean flag) {//写单个寄存器
        int mSalveId = ModbusUtils.checkSlave(salveId);
        final int mOffset = ModbusUtils.checkOffset(offset);
        final int mRegValue = ModbusUtils.checkRegValue(singleValue, hexadecimal);
        if (mSalveId != -1 && mOffset != -1 && mRegValue != -1) {
            ...
            ModbusManager.get()
                    .writeSingleRegister(mSalveId, mOffset, mRegValue,
                            new ModbusCallback<WriteRegisterResponse>() {
                                ...
                                public void onSuccess(WriteRegisterResponse writeRegisterResponse) {
//                                    showOneToast("F06写入成功\n");
                                    mModbusID3ManagerListener.WirteArmPositionID3(mOffset, "-1",mRegValue,null, flag);
                                }
                    ...
                    .writeRegisters(mSalveId, mOffset, mRegValues,
                            new ModbusCallback<WriteRegistersResponse>() {
                                ...
                                public void onSuccess(WriteRegistersResponse writeRegistersResponse) {
                                    // 发送成功
//                                    showOneToast("F16写入成功\n");
                                    mModbusID3ManagerListener.WirteArmPositionID3(mOffset, "-1",-1, mRegValues, true);
                                }
     ...
     *读保持寄存器
     */
    public void readManualDeviceID3(String salveId, String offset, String amount) {//读保持寄存器
        int mSalveId = ModbusUtils.checkSlave(salveId);
        final int mOffset = ModbusUtils.checkOffset(offset);
        int mAmount = ModbusUtils.checkAmount(amount);
        if (mSalveId != -1 && mOffset != -1 && mAmount != -1) {
...
//                            showOneToast("F03读取：\n" + ByteUtil.bytes2HexStr(data) + "\n");
                            if (mModbusID3ManagerListener != null) {
                                mModbusID3ManagerListener.ReadArmPositionID3(mOffset, ByteUtil.bytes2HexStr(data));
                            }
                        }

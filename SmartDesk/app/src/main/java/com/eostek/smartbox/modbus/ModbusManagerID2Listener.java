...
public class ModbusManagerID2Listener implements ModbusID2Manager.ModbusID2ManagerListener {
    ...
    private ModbusID2Manager mModbusManager;
    ...
    public ModbusManagerID2Listener(ModbusID2Manager mModbusID2Manager,  StationUiActivity stationUiActivity) {
        mModbusManager = mModbusID2Manager;
    ...
    @Override
    public void resetArmPositionID2(int mOffset, String readValue, int writeValue,short[] mRegValues, boolean state) {//重置机械臂位置
        if(mRegValues != null){
        ...
        if (mOffset == ModbusConstans.P201) {//257：自动模式；258：手动模式
            if (ByteUtil.hexStr2decimal(readValue) == ModbusConstans.P201_AUTO_MODE) {
                ...
                mModbusManager.showOneToast(mainActivity.getResources().getString(R.string.modbus_id2_manual_mode));
            }
            ...
            mModbusManager.showOneToast(mainActivity.getResources().getString(R.string.modbus_id2_warning) + ByteUtil.hexStr2decimal(readValue));
...
////          mModbusManager.restWirteDeviceP282Data0();//锁闸，往p282 写入数据0
    ...
    @Override
    public void middleWirteArmPositionID2(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state) {
        if(mRegValues != null){
...
//                    }
    ...
    @Override
    public void WirteArmPositionID2(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state) {
        if(mRegValues != null){
...
//                mModbusManager.WirteDeviceP290P291(ByteUtil.getHight(ModbusData.getNetworkDataID2()) + ","+ByteUtil.getLow(ModbusData.getNetworkDataID2()), true);
            } else {
                mModbusManager.writeDeviceP282(true);
            }
...
//                        DownloadwriteCount++;
//                        mainActivity.getMechanicalArmManager().writeArmPositionID2();
//                    }
                }
            } else {
                if (count < 15) {
    ...
    @Override
    public void ReadArmPositionID2(int mOffset, String value) {
        ...
        if (mOffset == ModbusConstans.P201) {//257：自动模式；258：手动模式
            ...
            SmartBoxInfo.setHightID2(hightNum);
            SmartBoxInfo.setLowID2(lowNum);
...
//            IsReadDeviceSuccess = true;
    ...
    private void ReadArmPositionNext(){
        mainActivity.getMechanicalArmManager().ReadArmPositionID3();
    }

    private long getOrigin(){
        return ModbusData.getOriginID2();// 8061384
    }

    private void setOrigin(long value){
        ModbusData.setOriginID2(value);
    }

    private long getMiddleValue(){
        return ModbusData.getMiddleID2();
    }

    private long geNetworkDataValue(){
        return ModbusData.getNetworkDataID2();
    }
    ...
    private String getMinddleData(){
        long Middle = ModbusData.getMiddleID2();
    ...
    private String getNetworkData(){
        long NetworkData = ModbusData.getNetworkDataID2();

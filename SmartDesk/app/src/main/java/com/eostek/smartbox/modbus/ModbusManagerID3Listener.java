...
public class ModbusManagerID3Listener implements ModbusID3Manager.ModbusID3ManagerListener {
    private static final String TAG = "modbus";

    private static final String ID = "ID3";

    private StationUiActivity mainActivity;

    private ModbusID3Manager mModbusManager;
    ...
    public ModbusManagerID3Listener(ModbusID3Manager mModbusID3Manager, StationUiActivity stationUiActivity) {
        mModbusManager = mModbusID3Manager;
    ...
    @Override
    public void resetArmPositionID3(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state) {//重置机械臂位置
        if(mRegValues != null){
        ...
        if (mOffset == ModbusConstans.P201) {//257：自动模式；258：手动模式
            if (ByteUtil.hexStr2decimal(readValue) == ModbusConstans.P201_AUTO_MODE) {
                ...
                mModbusManager.showOneToast(mainActivity.getResources().getString(R.string.modbus_id3_manual_mode));
            }
            ...
            mModbusManager.showOneToast(mainActivity.getResources().getString(R.string.modbus_id3_warning) + ByteUtil.hexStr2decimal(readValue));
...
//            Log.d(TAG, "reset ID3 P215: " + ByteUtil.hexStr2decimal(readValue));//456
//            restDevice = true;
////          mModbusManager.restWirteDeviceP282Data0();//锁闸，往p282 写入数据0
        }
    }
    ...
    @Override
    public void middleWirteArmPositionID3(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state) {
        if(mRegValues != null){
...
//                    }
    ...
    @Override
    public void WirteArmPositionID3(int mOffset, String readValue, int writeValue, short[] mRegValues, boolean state) {
        if(mRegValues != null){
...
//                mModbusManager.WirteDeviceP290P291(ByteUtil.getHight(ModbusData.getNetworkDataID3()) + ","+ByteUtil.getLow(ModbusData.getNetworkDataID3()), true);
            } else {
                mModbusManager.writeDeviceP282(true);
            }
...
//                        DownloadwriteCount++;
//                        mainActivity.getMechanicalArmManager().writeArmPositionID3();
//                    }
                }
            } else {
                if (count < 15) {
    ...
    @Override
    public void ReadArmPositionID3(int mOffset, String value) {
        ...
        if (mOffset == ModbusConstans.P201) {//257：自动模式；258：手动模式
            ...
            SmartBoxInfo.setHightID3(hightNum);
            SmartBoxInfo.setLowID3(lowNum);
...
//            IsReadDeviceSuccess = true;
    ...
    private void ReadArmPositionNext(){
        mainActivity.getMechanicalArmManager().ReadArmPositionID1();
    }

    private long getOrigin(){
        return ModbusData.getOriginID3();// 8061384
    }

    private void setOrigin(long value){
        ModbusData.setOriginID3(value);
    }

    private long getMiddleValue(){
        return ModbusData.getMiddleID3();
    }

    private long geNetworkDataValue(){
        return ModbusData.getNetworkDataID3();
    }
    ...
    private String getMinddleData(){
        long Middle = ModbusData.getMiddleID3();
    ...
    private String getNetworkData(){
        long NetworkData = ModbusData.getNetworkDataID3();
